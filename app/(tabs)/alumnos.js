...
export default function Alumnos() {
  ...
  //filtra por nombres y  matricula

  const alumnosFiltrados = alumnos.filter(
    //alumno es item del arreglo alumnos, filter regresa un nuevo arreglo con los elementos que cumplan la condicion dada
    (alumno) =>
      alumno.nombre.toLowerCase().includes(buscaAlumno.toLowerCase()) || //filtramos nombre del arreglo alumnos
      alumno.matricula.includes(buscaAlumno), // filtramos matricula del arreglo alumnos
  );
  // filter vs find: filter regresa un nuevo arreglo con los elementos que cumplan la condicion dada,
  // find regresa el primer elemento que cumpla la condicion dada

  if (!alumnos.length) {
    return <Text style={{ padding: 20 }}>Cargando alumnos...</Text>;
  }
...
//cambiar el orden sort, por nombre del alumno o apellidos
